...
const API_URL = "https://1ef2-112-135-71-163.in.ngrok.io/area-feedback";
const chart_array = [
  {
    ...
    axios
      ...
      .then((response) => {
        Object.keys(response.data).map((name) => {
          feedback = [];
          feedback.push(["Student", "emotions"]);
          Object.keys(response.data[name]).map((feedbackTitle) => {
            if (feedbackTitle !== "total") {
              feedback.push([feedbackTitle, response.data[name][feedbackTitle]]);
            }
            // console.log(feedbackTitle, response.data[name][feedbackTitle]);
          });
          //   console.log(feedback.length);
          areaDataSample.push({ division: name, data: feedback });
        });

        // console.log(areaDataSample);
        setAreaData(areaDataSample);
      })
      .catch((err) => {
        console.log(err);
      });
      ...
      <div className="container">
        ...
        <div className="cart_cards blu_bg pt-5 pb-4 px-3 d-flex align-items-center justify-content-center flex-wrap">
          {areaData.map((item, index) => (
            <div className="chart_card text-wrap my-3 p-2" key={index}>
              {console.log(item)}
              <h5>{item.division}</h5>
              <div className="chart d-flex align-items-center justify-content-center">
                <Chart className="cart_file" width="90%" height="90%" chartType="PieChart" color="#829ce9" loader={<div>Loading Pie Chart</div>} getDatafromArray areaData data={item.data} options={pieOptions} />
              </div>
            </div>
          ))}
        </div>
